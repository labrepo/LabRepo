...
var loadConfigFile = function (type) {
	...
	if(type == 'user') {
		var url = staticUrl + 'filemanager/scripts/filemanager.config.json';
	} else {
		var url = staticUrl + 'filemanager/scripts/filemanager.config.json.default';
	}
    ...
    json['options']['culture'] = document.getElementsByTagName("html")[0].getAttribute("lang").split('-')[0];
    return json;
};
...
config.options.fileRoot = fileRoot;
config.options.fileConnector = django_view;
...
loadCSS = function(href) {
	...
	if($.inArray(href, HEAD_included_files) == -1) {
		var cssLink = $("<link rel='stylesheet' type='text/css' href='" + staticUrl + 'filemanager/' + href + "'>");
		$("head").append(cssLink);
...
loadJS = function(src) {
	...
	if($.inArray(src, HEAD_included_files) == -1) {
		var jsLink = $("<script type='text/javascript' src='" + staticUrl + 'filemanager/' + src + "'>");
	    $("head").append(jsLink);
...
if($.urlParam('langCode') != 0 && file_exists (staticUrl + 'filemanager/scripts/languages/'  + $.urlParam('langCode') + '.js')) config.options.culture = $.urlParam('langCode');

var lg = [];
$.ajax({
  url: staticUrl + 'filemanager/scripts/languages/'  + config.options.culture + '.js',
  async: false,
  dataType: 'json',
...
//		template += '<button id="replace" name="replace" type="button" value="Replace">' + lg.replace + '</button>';
		...
		template += '<input id="mode" nam   e="mode" type="hidden" value="replace" /> ';
...
var getFolderInfo = function(path) {
	...
	$('#fileinfo').html('<img id="activity" src="' + staticUrl + 'filemanager/images/wait30trans.gif" width="30" height="30" />');
