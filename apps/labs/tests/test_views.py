...
class TestLabTest(TestCase):
    ...
    def test_create_test_lab(self):
        ...
        call_command('createtestlab', settings='setting.testing')
        ...
        self.assertEqual(Tag.objects.filter(lab=new_lab).distinct().count(), 24)
        self.assertEqual(Experiment.objects.filter(lab=new_lab).count(), 3)
        self.assertEqual(Unit.objects.filter(lab=new_lab, experiments__in=Experiment.objects.filter(lab=new_lab)).distinct().count(), 3)
        for unit in Unit.objects.filter(lab=new_lab):
            self.assertIsNotNone(unit.measurement, True)

        parent_tag1 = Tag.objects.filter(lab=new_lab, parent=None)
        self.assertEqual(parent_tag1.count(), 11)
        ...
        self.assertEqual(Tag.objects.filter(lab=lab).count(), 24)
        self.assertEqual(Experiment.objects.filter(lab=lab).count(), 3)
        self.assertEqual(Unit.objects.filter(lab=lab, experiments__in=Experiment.objects.filter(lab=lab)).distinct().count(), 3)
