    ...
    def extract_document(cls, obj_id, obj=None):
        ...
        return {
            ...
            'owners': [{'email': user.email,
                        ...
                        'last_name': user.last_name or ''} for user in obj.owners.all()],
            'editors': [{'email': user.email,
                         'first_name': user.first_name or '',
                         'last_name': user.last_name or ''} for user in obj.editors.all()],
            'viewers': [{'email': user.email,
                         'first_name': user.first_name or '',
                         'last_name': user.last_name or ''} for user in obj.viewers.all()],
            'lab': {
                'id': unicode(obj.lab.pk),
                'investigator': [{'email': user.email,
                                  'first_name': user.first_name or '',
                                  'last_name': user.last_name or ''} for user in obj.lab.investigator.all()],
                'members': [{'email': user.email,
                             'first_name': user.first_name or '',
                             'last_name': user.last_name or ''} for user in obj.lab.members.all()],
                'guests': [{'email': user.email,
                            'first_name': user.first_name or '',
                            'last_name': user.last_name or ''} for user in obj.lab.guests.all()],
            },
        }
